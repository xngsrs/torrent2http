...
func (c Config) parseFlags() {
    ...
    flag.IntVar(&config.requestTimeout, "request-timeout", 60, "The number of seconds until the current front piece request will time out")
    ...
    flag.IntVar(&config.minReconnectTime, "min-reconnect-time", 20, "The time to wait between peer connection attempts. If the peer fails, the time is multiplied by fail counter")
